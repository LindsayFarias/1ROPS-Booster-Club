...
describe('Booster club app\'s backend server', () => {
    ...
    test('should let you post a new committee for an event', (done) => {
        request(app)
            .post('/1rops/committee/2')
            .send({name: 'Brendan Kennedy'})
            .set('Accept', 'application/json')
            .then((res) => {
                expect(res.status).toBe(201);
    ...
    test('should let you remove committee members from an event', (done) => {
        request(app)
            .delete('/1rops/committee/1')
            .expect('Content-Type', /json/)
            .then(res => {
                expect(res.body).to.have.property('success', true);
            })
